...
class Engine(object):
    def __init__(self, state={}):
        ...
        # meters
        self.state['meter_loss'] = avgMeter.AverageValueMeter()
        #self.state['meter_loss'] = tnt.meter.AverageValueMeter()
        # time measure
        self.state['batch_time'] = avgMeter.AverageValueMeter()
        self.state['data_time'] = avgMeter.AverageValueMeter()
    ...
    def train(self, data_loader, model, criterion, optimizer, epoch):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            self.on_forward(True, model, criterion, data_loader, optimizer,False)
    ...
    def validate(self, data_loader, model, criterion):
        ...
        for i, (input, target) in enumerate(data_loader):
            ...
            if i % 10 == 0: 
                self.on_forward(False, model, criterion, data_loader, None,True)
            else:
                self.on_forward(False, model, criterion, data_loader, None,False)
...
class MultiLabelMAPEngine(Engine):
    ...
    def on_end_epoch(self, training, model, criterion, data_loader, optimizer=None, display=True):
        class_map = 100 * self.state['ap_meter'].value()
        map = 100 * self.state['ap_meter'].value().mean()
        loss = self.state['meter_loss'].value()[0]
        OP, OR, OF1, CP, CR, CF1, ACC = self.state['ap_meter'].overall()
        OP_k, OR_k, OF1_k, CP_k, CR_k, CF1_k, ACC_k = self.state['ap_meter'].overall_topk(3)
        if display:
            if training:
                print('Epoch: [{0}]\t'
                      'Loss {loss:.4f}\t'
                ...
                print('OP: {OP:.4f}\t'
                      ...
                      'CR: {CR:.4f}\t'
                      'CF1: {CF1:.4f}\t'
                      'ACC: {ACC:.4f}\t'.format(OP=OP, OR=OR, OF1=OF1, CP=CP, CR=CR, CF1=CF1,ACC=ACC))
            else:
                ...
                print('AP for each class:',class_map)
        return map
...
class GCNMultiLabelMAPEngine(MultiLabelMAPEngine):
    def on_forward(self, training, model, criterion, data_loader, optimizer=None, CAM = False,display=True):
